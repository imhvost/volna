		...
		Block::make( 'volna-' . $block_name, $block_title )
			...
			->set_render_callback(
				function ( $fields, $attributes ) use ( $block_name ) {
					...
					if ( $fields['volna_section_title'] || $fields['gallery'] ) :
						$block_id = uniqid();
						?>
						<div
						...
						>
							<div class="volna-container volna-slider-wrapp">
								...
								<?php
								if ( $fields['gallery'] ) :
									$gallery = array_chunk( $fields['gallery'], 2 );
									?>
									<div class="volna-gallery-slider swiper">
										<div class="swiper-wrapper">
										<?php foreach ( $gallery as $items ) : ?>
											<div class="volna-gallery-slide swiper-slide">
												<?php foreach ( $items as $item ) : ?>
													<a
														href="<?php echo esc_url( wp_get_attachment_image_url( $item, 'full' ) ); ?>"
														class="volna-gallery-link volna-cover-img glightbox"
